 ...
 * @return unsigned int The raw integer bits of the float.
 */
...
struct AABound
{
    Vec3 lo, hi; /**< Lower and upper bounds of the box. */
};
...
private:
    ...
    void init_empty()
    {
        ...
        tree.push_back((unsigned int)3 << 30);
...
public:
     ...
     *
     * @return unsigned int Number of primitives.
     */
    unsigned int primCount() { return objects.size(); }
    inline const Vec3& getBoundsMin() const { return bounds.low(); }
    inline const Vec3& getBoundsMax() const { return bounds.high(); }
    ...
    void intersectPoint(const Vec3& p, IsectCallback& intersectCallback) const
    {
...
protected:
    std::vector<unsigned int> tree; /**< The BIH tree structure. */
    std::vector<unsigned int> objects; /**< The objects in the BIH. */
    ...
    struct buildData
    {
        unsigned int* indices; /**< Indices of the primitives. */
        AABox* primBound; /**< Bounding boxes of the primitives. */
        unsigned int numPrims; /**< Number of primitives. */
