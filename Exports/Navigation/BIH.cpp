...
#include "BIH.h"
  ...
  */
void BIH::buildHierarchy(std::vector<unsigned int>& tempTree, buildData& dat, BuildStats& stats)
{
    // Create space for the first node (dummy leaf)
    tempTree.push_back((unsigned int)3 << 30);
 ...
 * @param stats Statistics for the build process.
 */
void BIH::subdivide(int left, int right, std::vector<unsigned int>& tempTree, buildData& dat, AABound& gridBox, AABound& nodeBox, int nodeIndex, int depth, BuildStats& stats)
{
        ...
        else
        {
            ...
            if (prevAxis != -1 && !isnan(prevClip))
            {
                ...
                if (wasLeft)
                {
                    ...
                    tempTree[nodeIndex + 2] = floatToRawIntBits(finf());
                }
                else
                {
                    ...
                    tempTree[nodeIndex + 1] = floatToRawIntBits(-finf());
...
bool BIH::WriteToFile(FILE* wf) const
{
    ...
    check += fwrite(&treeSize, sizeof(unsigned int), 1, wf);
    check += fwrite(&tree[0], sizeof(unsigned int), treeSize, wf);
    check += fwrite(&count, sizeof(unsigned int), 1, wf);
    check += fwrite(&objects[0], sizeof(unsigned int), count, wf);
