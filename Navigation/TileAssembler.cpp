...
// Specialization for pointer to ModelSpawn
template<>
struct BoundsTrait<VMAP::ModelSpawn*>
{
    static void getBounds(VMAP::ModelSpawn* obj, AABox& out)
    {
        out = obj->getBounds(); // Use -> instead of .
    }
};
...
namespace VMAP
{
        ...
        for (MapData::iterator map_iter = mapData.begin(); map_iter != mapData.end() && success; ++map_iter)
        {
            ...
            for (entry = map_iter->second->UniqueEntries.begin(); entry != map_iter->second->UniqueEntries.end(); ++entry)
            {
                ...
                else if (entry->second.flags & MOD_WORLDSPAWN) // WMO maps and terrain maps use different origin, so we need to adapt :/
                {
                    ...
                    entry->second.iBound = entry->second.iBound + Vec3(533.33333f * 32, 533.33333f * 32, 0.f);
                }
            ...
            std::map<unsigned int, unsigned int> modelNodeIdx;
            for (unsigned int i = 0; i < mapSpawns.size(); ++i)
            {
                modelNodeIdx.insert(pair<unsigned int, unsigned int>(mapSpawns[i]->ID, i));
            }
            ...
            unsigned int globalTileID = StaticMapTree::packTileID(65, 65);
            ...
            for (tile = tileEntries.begin(); tile != tileEntries.end(); ++tile)
            {
                ...
                unsigned int nSpawns = tileEntries.count(tile->first);
                ...
                if (success && fwrite(&nSpawns, sizeof(unsigned int), 1, tilefile) != 1)
                {
                    success = false;
                }
                // Write tile spawns
                for (unsigned int s = 0; s < nSpawns; ++s)
                {
                    ...
                    std::map<unsigned int, unsigned int>::iterator nIdx = modelNodeIdx.find(spawn2.ID);
                    if (success && fwrite(&nIdx->second, sizeof(unsigned int), 1, tilefile) != 1)
                    {
                        success = false;
                    }
                }
    ...
    bool TileAssembler::readMapSpawns()
    {
        ...
        unsigned int mapID, tileX, tileY, check = 0;
        ...
        while (!feof(dirf))
        {
            ...
            check += fread(&mapID, sizeof(unsigned int), 1, dirf);
            if (check == 0)
            {
                break;
            }
            ...
            check += fread(&tileX, sizeof(unsigned int), 1, dirf);
            check += fread(&tileY, sizeof(unsigned int), 1, dirf);
            ...
            current->UniqueEntries.insert(pair<unsigned int, ModelSpawn>(spawn.ID, spawn));
            // Associate the tile ID (constructed from tileX, tileY) with the spawn ID
            current->TileEntries.insert(pair<unsigned int, unsigned int>(StaticMapTree::packTileID(tileX, tileY), spawn.ID));
        }
 ...
 */
    bool GroupModel_Raw::Read(FILE* rf)
    {
        ...
        if (nindexes > 0)
        {
            ...
            unsigned short* indexarray = new unsigned short[nindexes];
            if (fread(indexarray, nindexes * sizeof(unsigned short), 1, rf) != 1)
            {
                fclose(rf);
                delete[] indexarray;
            ...
            for (unsigned int i = 0; i < nindexes; i += 3)
            {
        ...
        if (nvectors > 0)
        {
            ...
            for (unsigned int i = 0; i < nvectors; ++i)
            {
                vertexArray.push_back(Vec3(vectorarray + 3 * i));
            }
            delete[] vectorarray;
        }
    ...
    bool WorldModel_Raw::Read(const char* path, const char* RAW_VMAP_MAGIC)
    {
        ...
        unsigned int tempNVectors;
        READ_OR_RETURN(&tempNVectors, sizeof(tempNVectors));

        // Read the number of groups and the root WMO ID
        unsigned int groups;
        READ_OR_RETURN(&groups, sizeof(unsigned int));
        READ_OR_RETURN(&RootWMOID, sizeof(unsigned int));
        ...
        bool succeed = true;
        for (unsigned int g = 0; g < groups && succeed; ++g)
        {
