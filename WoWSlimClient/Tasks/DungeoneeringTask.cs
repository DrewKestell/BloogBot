...
namespace WoWSlimClient.Tasks.SharedStates
{
    public class DungeoneeringTask : BotTask, IBotTask
    {
        ...
        public void Update()
        {
            ...
            if (isPartyLeader)
            {
                ...
                if (CanProceed)
                {
                    ...
                    if (ObjectManager.Instance.Hostiles.Count(x => ObjectManager.Instance.Player.InLosWith(x) && x.Position.DistanceTo(ObjectManager.Instance.Player.Position) < 25) > 0)
                    {
                        ObjectManager.Instance.Player.StopAllMovement();

                        WoWUnit target = ObjectManager.Instance.Hostiles.Where(x => ObjectManager.Instance.Player.InLosWith(x)).OrderBy(x => Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, ObjectManager.Instance.Player.Position, x.Position, true)).First();
                        ...
                        ObjectManager.Instance.Player.SetTarget(target.Guid);
                    ...
                    else
                    {
                        if (NeedsGuidance)
                        {
                            if (destination.DistanceTo(ObjectManager.Instance.Player.Position) < 3)
                            {
                                SetNextWaypoint();
                            }
                        }
            ...
            else
            {
                ...
                {
                    Position[] locations = Navigation.Instance.CalculatePath(ObjectManager.Instance.MapId, ObjectManager.Instance.Player.Position, new Position(0, 0, 0), true);
                ...
                else if (ObjectManager.Instance.PartyLeader?.Position.DistanceTo(ObjectManager.Instance.Player.Position) > 15)
                {
                    Position[] locations = Navigation.Instance.CalculatePath(ObjectManager.Instance.MapId, ObjectManager.Instance.Player.Position, ObjectManager.Instance.PartyLeader.Position, true);
        ...
        private void SetNextWaypoint()
        {
            ...
            {
                destination = dungeonWaypoints.OrderBy(x => Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, ObjectManager.Instance.Player.Position, x, true)).First();
            }
            else
            {
        ...
        {
            List<Position> positions = dungeonWaypoints.Where(x => ObjectManager.Instance.Player.Position.InLosWith(x) && Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, ObjectManager.Instance.Player.Position, x, true) < 5).ToList();
        ...
        {
            Position[] locations = Navigation.Instance.CalculatePath(ObjectManager.Instance.MapId, ObjectManager.Instance.Player.Position, destination, true);

            if (locations.Length > 1)
            {
                currentWaypoint = locations[1];
                if (lastPosition != null && ObjectManager.Instance.Player.Position.DistanceTo(lastPosition) <= 0.05)
                ...
                lastPosition = ObjectManager.Instance.Player.Position;
        ...
        {
            Console.WriteLine($"[DUNGEONEERING TASK] Sorting encounter data");
            List<Creature> encounters = Container.MaNGOSDBClient.GetCreaturesByMapId((int)ObjectManager.Instance.MapId);
            majorWaypoints = [];
            minorWaypoints = [];
            ...
            majorWaypoints = [.. majorWaypoints.OrderBy(x => Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, destination, x, true))];
            ...
            for (int i = 0; i < majorWaypoints.Count; i++)
            {
                for (int j = 0; j < majorWaypoints.Count; j++)
                {
                    ...
                    {
                        weightedMatrix[i, j] = Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, majorWaypoints[i], majorWaypoints[j], true);
                    }
                }
            }
            ...
            for (int i = 0; i < dungeonWaypoints.Count; i++)
            {
                ...
                {
                    if (minorWaypoints.TryGetValue(majorWaypoints.OrderBy(x => Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, dungeonWaypoints[i], x, true)).First(), out List<Position> minorWaypointsList))
                    {
            ...
            {
                currentMinorWaypoint = minorWaypointsListFinal.OrderBy(x => Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, ObjectManager.Instance.Player.Position, x, true)).First();
        ...
        private List<Position> TravelingDungeonCrawler(List<Position> locations, int s)
        {
            ...
            for (int i = 0; i < locations.Count; i++)
            {
                for (int j = 0; j < locations.Count; j++)
                {
                    ...
                    {
                        graph[i, j] = Navigation.Instance.CalculatePathingDistance(ObjectManager.Instance.MapId, locations[i], locations[j], true);
                    }
                }
            }
        ...
        private bool NeedsGuidance => currentWaypoint.DistanceTo(ObjectManager.Instance.Player.Position) < 3 || !ObjectManager.Instance.Player.IsFacing(currentWaypoint) || !ObjectManager.Instance.Player.IsMoving;
