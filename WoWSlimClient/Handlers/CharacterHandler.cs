    ...
    public static class CharacterHandler
    {
        public static void HandleCharEnum(Opcodes opcode, byte[] data)
        {
            ...
            for (int i = 0; i < numChars; i++)
            {
                ...
                character.Equipment = new byte[20];
                for (int j = 0; j < 20; j++)
                {
            ...
            WoWEventHandler.Instance.FireOnCharacterListLoaded();
        }
        ...
        {
            WoWEventHandler.Instance.FireOnSetRestStart();
        }

        public static void HandleCharCreate(Opcodes opcode, byte[] data)
        {
            var result = (CreateCharacterResult)data[0];
            WoWEventHandler.Instance.FireOnCharacterCreateResponse(new CharCreateResponse(result));
        }

        public static void HandleCharDelete(Opcodes opcode, byte[] data)
        {
            var result = (DeleteCharacterResult)data[0];
            WoWEventHandler.Instance.FireOnCharacterDeleteResponse(new CharDeleteResponse(result));
        }
