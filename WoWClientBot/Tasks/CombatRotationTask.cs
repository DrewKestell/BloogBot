...
namespace WoWClientBot.AI.SharedStates
{
    public abstract class CombatRotationTask : BotTask
    {
        ...
        public bool Update(int desiredRange)
        {
            ...
            if (!Container.State.CharacterConfig.IsMainTank && ObjectManager.Aggressors.Any(x => x.TargetGuid == ObjectManager.Player.Guid))
            {
                MoveTowardsTank();
                return true;
            }
            ...
            if (!ObjectManager.Player.InLosWith(ObjectManager.Player.Target) || ObjectManager.Player.Position.DistanceTo(ObjectManager.Player.Target.Position) > desiredRange)
            {
                ...
                {
                    Position[] locations = Navigation.Instance.CalculatePath(ObjectManager.MapId, ObjectManager.Player.Position, ObjectManager.Player.Target.GetPointBehindUnit(3), true);
        ...
        public bool MoveTowardsTank()
        {
            if (raidLeader.Position.DistanceTo(ObjectManager.Player.Position) > 5)
            {
                ...
                Position[] locations = Navigation.Instance.CalculatePath(ObjectManager.MapId, ObjectManager.Player.Position, raidLeader.Position, true);
        ...
        public bool MoveBehindTarget(float distance)
        {
            ...
            Position[] locations = Navigation.Instance.CalculatePath(ObjectManager.MapId, ObjectManager.Player.Position, ObjectManager.Player.Target.GetPointBehindUnit(distance), true);
        ...
        public bool MoveBehindTankSpot(float distance)
        {
            ...
            Position[] locations = Navigation.Instance.CalculatePath(ObjectManager.MapId, ObjectManager.Player.Position, position, true);
