...
namespace WoWClientBot.Game.Frames
{
    public class LootFrame
    {
        ...
        public LootFrame()
        {
            var hasCoins = MemoryManager.ReadInt(MemoryAddresses.CoinCountPtr) > 0;
            if (hasCoins)
                LootItems.Add(new LootItem(null, 0, 0, true));
            for (var i = 0; i <= 15; i++)
            {
                var itemId = MemoryManager.ReadInt(MemoryAddresses.LootFrameItemsBasePtr + i * MemoryAddresses.LootFrameItemOffset);
                if (itemId == 0) break;
