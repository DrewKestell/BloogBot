...
namespace BotRunner.Tasks
{
    public class DungeoneeringTask : BotTask, IBotTask
    {
        ...
        public DungeoneeringTask(IBotContext botContext) : base( botContext)
        {
            if (isPartyLeader)
            {
                CreateWaypointMap();
            ...
            EventHandler.OnUnitKilled += Instance_OnUnitKilled;
        ...
        public void Update()
        {
            ...
            if (isPartyLeader)
            {
                ...
                if (CanProceed)
                {
                    ...
                    if (ObjectManager.Hostiles.Any(x => ObjectManager.Player.InLosWith(x) && x.Position.DistanceTo(ObjectManager.Player.Position) < 25))
                    {
                        ObjectManager.Player.StopAllMovement();

                        IWoWUnit target = ObjectManager.Hostiles.Where(x => ObjectManager.Player.InLosWith(x)).OrderBy(x => Container.PathfindingClient.GetPath(ObjectManager.MapId, ObjectManager.Player.Position, x.Position, true)).First();
            ...
            else
            {
                ...
                {
                    Position[] locations = Container.PathfindingClient.GetPath(ObjectManager.MapId, ObjectManager.Player.Position, new Position(0, 0, 0), true);
            ...
            {
                ObjectManager.Player.StopAllMovement();
                BotTasks.Push(Container.CreateRestTask(BotContext));
            }
            BotTasks.Push(Container.CreateBuffTask(BotContext));
        }
        ...
        private void SetNextWaypoint()
        {
            ...
            {
                destination = dungeonWaypoints.OrderBy(x => Container.PathfindingClient.GetPathingDistance(ObjectManager.MapId, ObjectManager.Player.Position, x, true)).First();
            }
            else
            {
                ObjectManager.Player.DoEmote(Emote.EMOTE_ONESHOT_CHEER);
                BotTasks.Pop();
                return;
            }
        }
        ...
        {
            List<Position> positions = dungeonWaypoints.Where(x => ObjectManager.Player.Position.InLosWith(x) && Container.PathfindingClient.GetPathingDistance(ObjectManager.MapId, ObjectManager.Player.Position, x, true) < 5).ToList();
        ...
        {
            Position[] locations = Container.PathfindingClient.GetPath(ObjectManager.MapId, ObjectManager.Player.Position, destination, true);
        ...
        {
            Console.WriteLine($"[DUNGEONEERING TASK] Sorting encounter data");
            List<Creature> encounters = Container.WoWDatabaseClient.GetCreaturesByMapId((int)ObjectManager.MapId);
            majorWaypoints = [];
            minorWaypoints = [];
            ...
            majorWaypoints = [.. majorWaypoints.OrderBy(x => Container.PathfindingClient.GetPathingDistance(ObjectManager.MapId, destination, x, true))];
            ...
            for (int i = 0; i < majorWaypoints.Count; i++)
            {
                for (int j = 0; j < majorWaypoints.Count; j++)
                {
                    ...
                    {
                        weightedMatrix[i, j] = Container.PathfindingClient.GetPathingDistance(ObjectManager.MapId, majorWaypoints[i], majorWaypoints[j], true);
                    }
                }
            }
            ...
            for (int i = 0; i < dungeonWaypoints.Count; i++)
            {
                ...
                {
                    if (minorWaypoints.TryGetValue(majorWaypoints.OrderBy(x => Container.PathfindingClient.GetPathingDistance(ObjectManager.MapId, dungeonWaypoints[i], x, true)).First(), out List<Position> minorWaypointsList))
                    {
            ...
            {
                currentMinorWaypoint = minorWaypointsListFinal.OrderBy(x => Container.PathfindingClient.GetPathingDistance(ObjectManager.MapId, ObjectManager.Player.Position, x, true)).First();
        ...
        private List<Position> TravelingDungeonCrawler(List<Position> locations, int s)
        {
            ...
            for (int i = 0; i < locations.Count; i++)
            {
                for (int j = 0; j < locations.Count; j++)
                {
                    ...
                    {
                        graph[i, j] = Container.PathfindingClient.GetPathingDistance(ObjectManager.MapId, locations[i], locations[j], true);
                    }
                }
            }
        ...
        private List<Position> GetWaypointsListFromEncounters(List<Creature> encounters)
        {
            ...
            for (int i = 0; i < encounters.Count; i++)
            {
                CreatureTemplate creatureTemplate = Container.WoWDatabaseClient.GetCreatureTemplateById(encounters[i].Id);
                List<CreatureGrouping> creatureLinkings = Container.WoWDatabaseClient.GetCreatureMappingByMemberGuid(encounters[i].Guid);
