...
namespace WoWSharpClient.Handlers
{
    internal class ObjectUpdateHandler
    {
        public static void HandleUpdateObject(Opcodes opcode, byte[] data)
        {
            ...
            Console.WriteLine($"{opcode}:{BitConverter.ToString(data)}");
        ...
        private static Models.Object CreateWoWObject(WoWObjectType objectType, byte[] guidBytes)
        {
            ...
            {
                WoWObjectType.None => new Models.Object(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                WoWObjectType.Item => new Item(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                WoWObjectType.Container => new Container(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                WoWObjectType.Unit => new Unit(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                WoWObjectType.Player => new Player(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                WoWObjectType.GameObj => new GameObject(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                WoWObjectType.DynamicObj => new DynamicObject(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                WoWObjectType.Corpse => new Corpse(guidBytes.Take(4).ToArray(), guidBytes.Skip(4).ToArray()),
                _ => null
            };
                ...
                if (maskMatch)
                {
                    switch ((EObjectFields)i)
                    {
                        ...
                        case EObjectFields.OBJECT_FIELD_PADDING:
                            ...
                            Console.WriteLine($"--[{(EObjectFields)i}]{BitConverter.ToString(objectFieldPadding)}");
                            break;
                    }
                }
            }
                ...
                if (maskMatch)
                {
                    switch ((EContainerFields)i)
                    {
                        ...
                        case EContainerFields.CONTAINER_FIELD_SLOT_LAST:
                            // These cases might be redundant since the slots are handled in the CONTAINER_FIELD_NUM_SLOTS case.
                            // If there is additional logic needed for individual slots, it should be added here.
                            break;
                        default:
                            Console.WriteLine($"Unhandled container field: {i}");
                            break;
                    }
                }
            }
        ...
        private static void ReadUnitField(BinaryReader reader, Unit wowUnit, EUnitFields field)
        {
            switch (field)
            {
                ...
                case EUnitFields.UNIT_FIELD_HEALTH:
                    wowUnit.Health = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.Health}");
                    break;
                case EUnitFields.UNIT_FIELD_POWER1:
                    wowUnit.Power[Powers.MANA] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.Power[Powers.MANA]}");
                    break;
                case EUnitFields.UNIT_FIELD_POWER2:
                    wowUnit.Power[Powers.RAGE] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.Power[Powers.RAGE]}");
                    break;
                case EUnitFields.UNIT_FIELD_POWER3:
                    wowUnit.Power[Powers.ENERGY] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.Power[Powers.ENERGY]}");
                    break;
                case EUnitFields.UNIT_FIELD_POWER4:
                    wowUnit.Power[Powers.FOCUS] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.Power[Powers.FOCUS]}");
                    break;
                case EUnitFields.UNIT_FIELD_POWER5:
                    wowUnit.Power[Powers.HAPPINESS] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.Power[Powers.HAPPINESS]}");
                    break;
                case EUnitFields.UNIT_FIELD_MAXHEALTH:
                    wowUnit.MaxHealth = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.MaxHealth}");
                    break;
                case EUnitFields.UNIT_FIELD_MAXPOWER1:
                    wowUnit.MaxPower[Powers.MANA] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.MaxPower[Powers.MANA]}");
                    break;
                case EUnitFields.UNIT_FIELD_MAXPOWER2:
                    wowUnit.MaxPower[Powers.RAGE] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.MaxPower[Powers.RAGE]}");
                    break;
                case EUnitFields.UNIT_FIELD_MAXPOWER3:
                    wowUnit.MaxPower[Powers.ENERGY] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.MaxPower[Powers.ENERGY]}");
                    break;
                case EUnitFields.UNIT_FIELD_MAXPOWER4:
                    wowUnit.MaxPower[Powers.FOCUS] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.MaxPower[Powers.FOCUS]}");
                    break;
                case EUnitFields.UNIT_FIELD_MAXPOWER5:
                    wowUnit.MaxPower[Powers.HAPPINESS] = reader.ReadUInt32();
                    Console.WriteLine($"--[{field}]: {wowUnit.MaxPower[Powers.HAPPINESS]}");
                    break;
                case EUnitFields.UNIT_FIELD_LEVEL:
                    wowUnit.Level = reader.ReadUInt32();
